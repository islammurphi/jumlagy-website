...
function loadDashboard() {
    ...
    if (ordersCtx && ordersCtx.parentElement && ordersCtx.parentElement.parentElement) {
        ordersCtx.parentElement.parentElement.style.display = 'none';
    }
}
...
function loadAlerts() {
    ...
    document.getElementById('alerts-summary').innerHTML = `<div class="stat-card bg-red-50"><p class="text-xs text-gray-500">تأخر تسليم</p><p class="text-3xl font-bold text-red-600">${allAlerts.filter(a=>a.type==='overdue').length}</p></div><div class="stat-card bg-amber-50"><p class="text-xs text-gray-500">مخزون منخفض</p><p class="text-3xl font-bold text-amber-600">${allAlerts.filter(a=>a.type==='stock').length}</p></div>`;
