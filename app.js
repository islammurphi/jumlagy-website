...
async function printRepairInvoice(repairId) {
    ...
    printWindow.document.write(`
        <html dir="rtl"><head><title>فاتورة صيانة - ${repair.device_name}</title>
        <style>@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;800&display=swap');
        body{font-family:'Tajawal',sans-serif;padding:30px;color:#1e293b}h1{color:#2563eb;font-size:22px}hr{border:1px solid #e2e8f0;margin:16px 0}.info{margin:8px 0;font-size:14px}.total{font-size:22px;color:#2563eb;font-weight:800;text-align:center;margin:20px 0}.stamp{margin-top:50px;text-align:left}@media print{body{padding:10px}}</style></head>
        <body><h1>${shopName}</h1><p>📞 ${phone}</p><hr>
        <p class="info"><b>العميل:</b> ${repair.customer_name}</p>
        <p class="info"><b>الجهاز:</b> ${repair.device_name}</p>
        <p class="info"><b>الفني:</b> ${repair.technician || '—'}</p>
        <p class="info"><b>تاريخ الاستلام:</b> ${repair.receive_date || '—'}</p>
        <p class="info"><b>المشكلة:</b> ${repair.device_issue || '—'}</p>
        ${repair.spare_part_name ? `<p class="info"><b>قطع الغيار:</b> ${repair.spare_part_name}</p>` : ''}
        <div class="total">💰 ${formatCurrency(repair.repair_price)}</div>
        <p style="font-size:12px;color:#64748b;">🛡️ الضمان: ${warrantyDays} يوم</p>
        <div class="stamp"><p>توقيع العميل: _______________</p></div>
