...
// ================================
// ⭐ عرض الدوال للنطاق العام فوراً (قبل أي حاجة)
// ================================
...
async function loadAllData() {
    ...
    try {
        ...
        if (settingsDoc.exists()) {
            ...
            globalSettings = { shop_name: 'jumlazy', owner_name: 'اسم حسن', phone: '01207696202', address: 'المقطر', warranty_days: 30, warranty_notes: 'تبني', language: 'ar', technicians: globalTechnicians };
...
function loadDashboard() {
    ...
    if (globalUsers.length > 0) {
        document.getElementById('users-manager').innerHTML = `
            ...
            <div class="space-y-2">
                ${globalUsers.map(u => `<div class="flex justify-between items-center bg-gray-50 rounded-lg px-3 py-2"><span class="font-medium text-sm">${u.fullName || u.name || u.email}</span><span class="text-xs text-gray-500">${u.email}</span><span class="badge ${u.role === 'admin' ? 'badge-blue' : 'badge-gray'}">${u.role === 'admin' ? 'مدير' : 'مستخدم'}</span><span class="text-xs text-gray-500">${u.subscriptionType || u.subscription?.plan || '-'} - ${u.subscriptionEnd || u.subscription?.end_date || ''}</span></div>`).join('')}
            </div>
        `;
    }
    ...
    document.getElementById('expenses-list').innerHTML = filtered.map(e => `
        <div class="card"><div class="card-body"><div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div><p class="font-semibold">${e.title}</p><p class="text-xs text-gray-500">${e.date} · ${e.category}${e.notes ? ' — ' + e.notes : ''}</p></div>
            </div>
            <div class="flex items-center gap-3">
...
function loadWallets() {
    ...
    document.getElementById('wallets-table-body').innerHTML = globalWallets.map(w => {
        ...
        return `<tr>
            ...
            <td><span class="badge badge-blue">${walletLimits[w.type]?.label || w.type}</span></td>
...
async function saveTransaction(e) {
    ...
    if (type === 'withdraw') {
        ...
        if ((wallet.daily_used + amount) > wallet.daily_limit) {
            warningDiv.textContent = `❌ هذه العملية تتجاوز الحد اليومي (${formatCurrency(wallet.daily_limit)}).`;
            warningDiv.classList.remove('hidden');
            return;
        }
        if ((wallet.monthly_used + amount) > wallet.monthly_limit) {
