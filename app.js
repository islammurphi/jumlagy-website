...
const App = (() => {
    ...
    async function init() {
        ...
        try {
            ...
            // تهيئة Firebase Auth واستعادة الجلسة إن وجدت
            let initialUser = null;
            try {
                initialUser = await Auth.init();
            } catch (e) {
                console.error('❌ Auth init failed:', e);
                UIRenderer.showToast(e?.message || 'فشل تهيئة تسجيل الدخول', 'error');
            }

            if (initialUser) await showDashboard();
    ...
    function showLoginScreen() {
        ...
        // لا يوجد حقول دخول في وضع Google-only
    }
    ...
    function setupGlobalEventListeners() {
        ...
        // القائمة الجانبية (delegation لأن العناصر تُبنى ديناميكياً)
    ...
    async function handleLogin() {
        try {
            ...
            UIRenderer.showToast(`مرحباً ${user?.fullName || 'بك'} 👋`, 'success');
            await showDashboard();
        } catch (error) {
            ...
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="fab fa-google" aria-hidden="true"></i> متابعة باستخدام Google';
            }
        }
    }
