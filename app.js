...
async function seedDemoData() {
    ...
    const demoRepairs = [
        ...
        { device_name: 'Samsung S24 Ultra', customer_name: 'محمود علي', customer_phone: '01007654321', technician: 'تحن', status: 'قيد_الصيانة', repair_price: 1800, technician_fee: 300, spare_part_name: 'بطارية', spare_part_cost: 800, receive_date: '2026-04-20', delivery_date: null, device_issue: 'بطارية ضعيفة', notes: 'انتظار قطعة الغيار', ownerId },
...
async function printRepairInvoice(repairId) {
    ...
    if (!repair) return alert('❌ أمر الصيانة غير موجود');
    ...
    printWindow.document.write(`
        <html dir="rtl"><head><title>فاتورة صيانة</title>
        <style>@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;800&display=swap');
        body{font-family:'Tajawal',sans-serif;padding:30px}h1{color:#2563eb}table{width:100%;border-collapse:collapse}td{padding:8px;border-bottom:1px solid #eee}.total{font-size:20px;color:#2563eb;font-weight:800}@media print{body{padding:10px}}</style></head>
        <body><h1>${shopName}</h1><p>📞 ${phone}</p><hr>
        <p><b>العميل:</b> ${repair.customer_name} | <b>الجهاز:</b> ${repair.device_name}</p>
        <p><b>الفني:</b> ${repair.technician} | <b>التاريخ:</b> ${repair.receive_date}</p>
        <p><b>المشكلة:</b> ${repair.device_issue || '—'}</p>
        <p class="total">💰 ${formatCurrency(repair.repair_price)}</p>
        <p style="margin-top:40px">توقيع العميل: _______________</p>
        ...
        container.innerHTML = `
            <div class="table-responsive"><table><thead><tr>
                ...
                <tr>
                    ...
                    <td>${r.technician || '-'}</td>
                    <td>
                        <select class="input-field" style="width:auto;padding:4px 8px;font-size:12px;" onchange="quickStatusChange('${r.id}', this.value)">
                    ...
                    <td>
                        ...
                        <button class="btn-icon" onclick="printRepairInvoice('${r.id}')" title="طباعة"><i class="fas fa-print"></i></button>
...
function loadCustomersTable() {
    ...
    let customers = Object.values(customerMap).map((c, idx) => ({
        ...c, id: idx, lastVisit: c.lastDate ? c.lastDate.toISOString().split('T')[0] : '-',
...
function loadWallets() {
    ...
    if (tableBody) {
        tableBody.innerHTML = globalWallets.length ? globalWallets.map(w => {
            let limitStatus = '<span class="badge badge-green">آمن</span>';
            return `<tr>
                ...
                <td>
                    ...
                    <button class="btn-icon" onclick="openWalletModal('${w.id}')"><i class="fas fa-pen"></i></button>
    ...
    if (transactionsBody) {
        transactionsBody.innerHTML = sorted.length ? sorted.slice(0, 20).map(t => {
            ...
            return `<tr>
                ...
                <td class="font-bold">${t.type === 'deposit' ? '+' : '-'} ${formatCurrency(t.amount)}</td>
...
function onWalletTypeChange() {
    ...
    if (type && walletLimits[type] && info) {
        ...
        info.innerHTML = `الحد اليومي: <strong>${walletLimits[type].daily.toLocaleString()} ج.م</strong> | الحد الشهري: <strong>${walletLimits[type].monthly.toLocaleString()} ج.م</strong>`;
    } else if (info) {
...
async function saveWallet(e) {
    ...
    const data = {
        ...
        daily_limit: limits.daily, monthly_limit: limits.monthly, ownerId,
    };
    try {
        if (id) {
...
async function saveTransaction(e) {
    ...
    try {
        if (type === 'withdraw') {
            await updateDoc(doc(db, "wallets", walletId), { balance: Number(wallet.balance) - amount, daily_used: Number(wallet.daily_used) + amount, monthly_used: Number(wallet.monthly_used) + amount });
        } else {
...
function loadReports() {
    ...
    if (kpiEl) {
        kpiEl.innerHTML = `
            <div class="stat-card"><p class="stat-card-title">إجمالي الإيرادات</p><p class="stat-card-value">${formatCurrency(totalRevenue)}</p><p class="stat-card-sub">${globalRepairs.length} عملية</p></div>
            ...
            <div class="stat-card"><p class="stat-card-title">متوسط قيمة الطلب</p><p class="stat-card-value">${formatCurrency(avgOrderValue)}</p></div>
            <div class="stat-card"><p class="stat-card-title">معدل إتمام الصيانة</p><p class="stat-card-value">${completionRate}%</p></div>
        `;
    }
    ...
    document.getElementById('top-customers').innerHTML = Object.values(custMap).length ? Object.values(custMap).sort((a, b) => b.total - a.total).slice(0, 8).map((c, i) => `
    ...
    document.getElementById('top-devices').innerHTML = Object.values(deviceMap).length ? Object.values(deviceMap).sort((a, b) => b.count - a.count).slice(0, 8).map((d, i) => `
        <div class="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2.5 mb-2"><span class="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center text-xs font-bold text-purple-600">${i + 1}</span><div class="flex-1"><p class="font-semibold text-sm">${d.name}</p></div><div class="text-sm font-bold">${d.count} جهاز</div></div>
    `).join('') : '<p class="text-gray-400 text-center py-4">لا توجد بيانات</p>';
}
...
function loadAlerts() {
    const now = new Date();
    const allAlerts = [...globalParts.filter(p => p.min_quantity && p.quantity <= p.min_quantity).map(p => ({ type:'stock', title:`مخزون منخفض: ${p.name}`, desc:`الكمية المتبقية ${p.quantity}`, icon:'fa-box', color:'text-amber-600', bg:'bg-amber-50', border:'border-r-amber-400' })), ...globalRepairs.filter(r => r.delivery_date && r.status !== 'تم_التسليم' && new Date(r.delivery_date) < now).map(r => ({ type:'overdue', title:`تأخر تسليم: ${r.device_name}`, desc:`العميل: ${r.customer_name}`, icon:'fa-clock', color:'text-red-600', bg:'bg-red-50', border:'border-r-red-500' }))];
    ...
    document.getElementById('alerts-summary').innerHTML = `
        <div class="stat-card bg-red-50"><p class="text-xs text-gray-500">تأخر تسليم</p><p class="text-3xl font-bold text-red-600">${allAlerts.filter(a=>a.type==='overdue').length}</p></div>
        <div class="stat-card bg-amber-50"><p class="text-xs text-gray-500">مخزون منخفض</p><p class="text-3xl font-bold text-amber-600">${allAlerts.filter(a=>a.type==='stock').length}</p></div>
    `;
    document.getElementById('alerts-list').innerHTML = allAlerts.length > 0 ? allAlerts.map(a => `
        <div class="card ${a.bg} border-r-4 ${a.border}"><div class="card-body"><div class="flex items-start gap-3"><div class="p-2 rounded-lg"><i class="fas ${a.icon} ${a.color} text-lg"></i></div><div><p class="font-bold">${a.title}</p><p class="text-sm text-gray-600">${a.desc}</p></div></div></div></div>
    `).join('') : '<div class="card"><div class="card-body text-center py-10"><i class="fas fa-check-circle text-emerald-500 text-4xl mb-3"></i><p class="text-lg font-bold text-emerald-700">كل شيء على ما يرام!</p></div></div>';
}
...
function loadSubscriptions() {
    ...
    document.getElementById('subscription-summary-cards').innerHTML = `
        ...
        <div class="stat-card"><div class="stat-card-icon icon-blue"><i class="fas fa-dollar-sign"></i></div><p class="stat-card-title">الإيرادات</p><p class="stat-card-value">${formatCurrency(globalSubscriptions.reduce((s,sub)=>s+(Number(sub.price)||0),0))}</p></div>
    `;
    ...
    if (tableBody) {
        tableBody.innerHTML = filtered.length ? filtered.map((s, i) => `<tr>
            <td class="text-xs text-gray-400">${i+1}</td><td class="font-semibold">${s.customer_name||'غير محدد'}</td><td class="text-sm">${s.customer_email||'-'}</td>
            <td>${s.plan||'-'}</td><td class="font-bold text-blue-600">${formatCurrency(s.price)}</td>
            <td class="text-sm">${s.start_date||'-'}</td><td class="text-sm">${s.end_date||'-'}</td><td>${getDaysLeft(s.end_date)}</td>
            <td>${s.status==='نشط'?'<span class="badge badge-green">نشط</span>':'<span class="badge badge-red">منتهي</span>'}</td>
            <td>${(s.status==='منتهي'||s.status==='نشط')?`<button class="btn-primary btn-xs" onclick="renewSubscription('${s.id}')"><i class="fas fa-sync-alt"></i></button>`:''}
            <button class="btn-danger btn-xs" onclick="confirmDelete('subscription','${s.id}')"><i class="fas fa-trash"></i></button></td></tr>`
        ).join('') : '<tr><td colspan="10" class="text-center py-6 text-gray-400">لا توجد اشتراكات</td></tr>';
    }
}
...
function openSubscriptionModal(subId = null) {
    ...
    if (select) select.innerHTML = '<option value="">اختر مستخدم</option>' + globalUsers.map(u => `<option value="${u.id}">${u.fullName||u.name||u.email} (${u.email})</option>`).join('');
    if (subId) {
...
function onLinkedUserChange() {
    ...
    if (userId) { const user = globalUsers.find(u => u.id === userId); if (user) { document.getElementById('subscription-customer-name').value = user.fullName||user.name||''; document.getElementById('subscription-customer-email').value = user.email||''; } }
}
    ...
    try {
        ...
        if (data.linked_user_id) {
            await updateDoc(doc(db, "users", data.linked_user_id), {
                ...
                subscriptionType: data.plan, subscriptionEnd: data.end_date, isApproved: true, status: 'active'
            });
        }
        await loadAllData();
...
function renderTechnicians() {
    ...
    list.innerHTML = globalTechnicians.length ? globalTechnicians.map((t, i) => `
        ...
        </div>`).join('') : '<p class="text-sm text-gray-500">لم تضف فنيين بعد</p>';
}
...
function confirmDelete(type, id) {
    ...
    document.getElementById('delete-message').textContent = 'هل أنت متأكد من الحذف؟';
}
