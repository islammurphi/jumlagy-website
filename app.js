...
import { getFirestore, collection, doc, setDoc, getDoc, getDocs, addDoc, updateDoc, deleteDoc, query, where, orderBy, serverTimestamp, onSnapshot, writeBatch } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
// دوال مساعدة
// ================================
function formatCurrency(amount) { return Number(amount).toLocaleString('ar-EG') + ' ج.م'; }
...
async function initApp() {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (!user) {
            // لا يوجد مستخدم - توجيه لصفحة تسجيل الدخول
        ...
        if (!userDoc.exists()) {
            // مستخدم جديد - إنشاء اشتراك تجريبي
            ...
            if (sub.status === 'نشط' && endDate < today) {
                // الاشتراك انتهى
            ...
            currentSession = {
                ...
                plan: sub.plan || 'لا يوجد',
                end_date: sub.end_date || '',
            };
        }
        
        // التأكد من صلاحية الاشتراك
        if (currentSession.plan !== 'تجريبي' && currentSession.plan !== 'شهري' && currentSession.plan !== 'سنوي') {
            if (currentSession.end_date && new Date(currentSession.end_date) < new Date()) {
                // الاشتراك منتهي - توجيه لصفحة تسجيل الدخول
                window.location.href = 'login.html';
                return;
            }
        }
...
function updateUserUI() {
    ...
    document.getElementById('sidebar-user-name').textContent = currentSession.name;
    document.getElementById('sidebar-user-role').textContent = currentSession.role === 'admin' ? 'مدير النظام' : `مشترك - ${currentSession.plan}`;
    document.getElementById('sidebar-user-photo').src = currentSession.photo || '';
}

function checkSubscriptionBanner() {
    if (!currentSession || !currentSession.end_date) return;
...
// ================================
// تحميل كل البيانات من Firestore
// ================================
async function loadAllData() {
    if (!ownerId) return;
    
    try {
        const [repairsSnap, partsSnap, expensesSnap, walletsSnap, transactionsSnap, subscriptionsSnap, settingsSnap] = await Promise.all([
            getDocs(query(collection(db, "repairs"), where("ownerId", "==", ownerId), orderBy("receive_date", "desc"))),
            getDocs(query(collection(db, "parts"), where("ownerId", "==", ownerId), orderBy("name", "asc"))),
            getDocs(query(collection(db, "expenses"), where("ownerId", "==", ownerId), orderBy("date", "desc"))),
            getDocs(query(collection(db, "wallets"), where("ownerId", "==", ownerId))),
            getDocs(query(collection(db, "transactions"), where("ownerId", "==", ownerId), orderBy("date", "desc"))),
        ...
        if (settingsSnap.exists()) {
            ...
            globalSettings = { shop_name: 'jumlazy', owner_name: 'اسم حسن', phone: '01207696202', address: 'المقطر', warranty_days: 30, warranty_notes: 'تبني', language: 'ar', logo: '' };
        ...
        if (!globalSettings.technicians) {
            await updateDoc(doc(db, "settings", ownerId), { technicians: globalTechnicians });
        }
...
function initEvents() {
    ...
    // التاريخ
    const today = new Date();
    document.getElementById('current-date').textContent = today.toLocaleDateString('ar-EG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
...
function loadDashboard() {
    const totalRevenue = globalRepairs.reduce((s, r) => s + (r.repair_price || 0), 0);
    const totalPartsCost = globalRepairs.reduce((s, r) => s + (r.spare_part_cost || 0), 0);
    const totalTechFees = globalRepairs.reduce((s, r) => s + (r.technician_fee || 0), 0);
    const totalExpenses = globalExpenses.reduce((s, e) => s + (e.amount || 0), 0);
    const totalCosts = totalPartsCost + totalTechFees + totalExpenses;
    const totalProfit = totalRevenue - totalCosts;
    const inventoryValue = globalParts.reduce((s, p) => s + (p.purchase_price || 0) * (p.quantity || 0), 0);
    ...
    document.getElementById('recent-repairs').innerHTML = recent.map(r => `
    ...
    if (currentSession.role === 'admin') {
        document.getElementById('users-manager').innerHTML = `
            <div class="space-y-2">
                ${globalUsers.map(u => `<div class="flex justify-between items-center bg-gray-50 rounded-lg px-3 py-2"><span class="font-medium text-sm">${u.name || u.email}</span><span class="text-xs text-gray-500">${u.email}</span><span class="badge ${u.role === 'admin' ? 'badge-blue' : 'badge-gray'}">${u.role === 'admin' ? 'مدير' : 'مستخدم'}</span><span class="text-xs text-gray-500">${u.subscription?.plan || 'لا يوجد'} - ${u.subscription?.end_date || ''}</span></div>`).join('')}
            </div>
        `;
    }
...
function loadDashboardCharts() {
    ...
    charts.orders = new Chart(ordersCtx, {
        type: 'doughnut',
        data: { labels: ['تم التسليم', 'قيد الصيانة', 'جاهز للتسليم'], datasets: [{ data: [statusCounts['تم_التسليم'], statusCounts['قيد_الصيانة'], statusCounts['جاهز']], backgroundColor: ['#3b82f6', '#f59e0b', '#10b981'], borderWidth: 0 }] },
    ...
    const monthlyRevenue = months.map(m => globalRepairs.filter(r => r.receive_date?.includes(m === 'يناير' ? '01' : m === 'فبراير' ? '02' : m === 'مارس' ? '03' : m === 'أبريل' ? '04' : m === 'نوفمبر' ? '11' : '12')).reduce((s, r) => s + (r.repair_price || 0), 0));
    const monthlyExpenses = months.map(m => globalExpenses.filter(e => e.date?.includes(m === 'يناير' ? '01' : m === 'فبراير' ? '02' : m === 'مارس' ? '03' : m === 'أبريل' ? '04' : m === 'نوفمبر' ? '11' : '12')).reduce((s, e) => s + (e.amount || 0), 0));
    
    charts.income = new Chart(incomeCtx, {
        type: 'line',
        data: { labels: months, datasets: [
            { label: 'الإيرادات', data: monthlyRevenue.length ? monthlyRevenue : [3000, 4500, 6000, 7000, 8000, 9130], borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.08)', fill: true, tension: 0.4 },
            { label: 'المصاريف', data: monthlyExpenses.length ? monthlyExpenses : [400, 300, 500, 200, 300, 55], borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.08)', fill: true, tension: 0.4 }
        ] },
...
function loadRepairsTable() {
    ...
    let filtered = globalRepairs.filter(r => {
        const match = !search || r.device_name?.toLowerCase().includes(search) || r.customer_name?.toLowerCase().includes(search);
...
function openPartForm(partId = null) {
    ...
    if (partId) {
        ...
        if (p) {
            ...
            document.getElementById('part-purchase-price').value = p.purchase_price;
            document.getElementById('part-selling-price').value = p.selling_price || '';
            document.getElementById('part-quantity').value = p.quantity;
            document.getElementById('part-min-quantity').value = p.min_quantity || '';
...
function loadInventoryTable() {
    ...
    const totalValue = globalParts.reduce((s, p) => s + (p.purchase_price || 0) * (p.quantity || 0), 0);
...
function loadExpensesTable() {
    ...
    const filtered = globalExpenses.filter(e => {
        const match = !search || e.title?.toLowerCase().includes(search);
        return match && (cat === 'الكل' || e.category === cat);
    });
    const total = globalExpenses.reduce((s, e) => s + (e.amount || 0), 0);
    const recurring = globalExpenses.filter(e => e.is_recurring).reduce((s, e) => s + (e.amount || 0), 0);
    ...
    document.getElementById('expenses-list').innerHTML = filtered.map(e => `
        <div class="card"><div class="card-body"><div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div><p class="font-semibold">${e.title}</p><p class="text-xs text-gray-500">${e.date} · ${e.category}</p></div>
            </div>
            <div class="flex items-center gap-3">
...
function loadCustomersTable() {
    ...
    globalRepairs.forEach(r => {
        ...
        customerMap[key].totalPaid += (r.repair_price || 0);
    ...
    let customers = Object.values(customerMap).map((c, idx) => ({ ...c, id: idx, lastVisit: c.lastDate ? c.lastDate.toISOString().split('T')[0] : '-' })).sort((a, b) => (b.lastDate || 0) - (a.lastDate || 0));
    ...
    document.getElementById('customers-summary').innerHTML = `
        ...
        ${topCustomer ? `<div class="stat-card"><div class="stat-card-icon icon-amber"><i class="fas fa-star"></i></div><p class="stat-card-title">الأكثر تعاملاً</p><p class="stat-card-value text-base">${topCustomer.name}</p><p class="stat-card-sub">${topCustomer.repairs.length} جهاز</p></div>` : ''}
    `;
            ...
            <div class="card-body">
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        ...
                        <div><p class="font-bold">${c.name}</p><p class="text-sm text-gray-500">📞 ${c.phone || '-'}</p></div>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="text-center hidden sm:block"><p class="text-xs text-gray-400">عدد الأجهزة</p><p class="font-bold">${c.repairs.length}</p></div>
                        <div class="text-center hidden sm:block"><p class="text-xs text-gray-400">إجمالي المدفوع</p><p class="font-bold text-blue-600">${formatCurrency(c.totalPaid)}</p></div>
                        <div class="text-center hidden sm:block"><p class="text-xs text-gray-400">آخر زيارة</p><p class="text-sm">${c.lastVisit}</p></div>
                ...
                <div class="customer-repairs mt-3 pt-3 hidden" id="customer-repairs-${c.id}">
                    ...
                    ${c.repairs.map(r => `
                        <div class="customer-repair-item">
                            <div class="flex justify-between items-center">
                                <div><p class="font-semibold text-sm">${r.device_name}</p><p class="text-xs text-gray-500">${r.receive_date} · ${r.technician}</p></div>
...
function loadWallets() {
    const totalBalance = globalWallets.reduce((s, w) => s + (w.balance || 0), 0);
    const dailyTotal = globalWallets.reduce((s, w) => s + (w.daily_used || 0), 0);
    const monthlyTotal = globalWallets.reduce((s, w) => s + (w.monthly_used || 0), 0);
    ...
    document.getElementById('wallets-table-body').innerHTML = globalWallets.map(w => {
        ...
        let limitStatus = (w.daily_used >= w.daily_limit || w.monthly_used >= w.monthly_limit) ? '<span class="badge badge-red">⚠️ تعدى الحد</span>'
            : (pctDaily >= 80 || pctMonthly >= 80) ? '<span class="badge badge-amber">قرب من الحد</span>'
            : '<span class="badge badge-green">آمن</span>';
        return `<tr>
            <td class="font-semibold">${w.name}</td><td><span class="badge badge-blue">${walletLimits[w.type]?.label || w.type}</span></td>
            <td class="font-bold">${formatCurrency(w.balance)}</td><td>${formatCurrency(w.daily_limit)}</td>
            <td>${formatCurrency(w.daily_used)}</td><td>${formatCurrency(w.monthly_limit)}</td>
            <td>${formatCurrency(w.monthly_used)}</td><td>${limitStatus}</td>
            <td>
    ...
    document.getElementById('wallet-transactions-body').innerHTML = sorted.slice(0, 20).map(t => {
        const w = globalWallets.find(w => w.id === t.wallet_id);
        return `<tr>
            <td class="text-sm">${t.date}</td><td class="font-semibold">${w ? w.name : '—'}</td>
            <td>${t.type === 'deposit' ? '<span class="badge badge-green">إيداع</span>' : '<span class="badge badge-red">سحب</span>'}</td>
            <td class="font-bold">${formatCurrency(t.amount)}</td><td class="text-sm text-gray-500">${t.notes || '—'}</td></tr>`;
...
async function saveWallet(e) {
    ...
    const data = {
        ...
        max_balance: limits.max_balance, alert_threshold: Math.round(limits.monthly * 0.8),
        ownerId: ownerId,
    };
    try {
        if (id) {
...
async function saveTransaction(e) {
    ...
    if (type === 'withdraw') {
        if (amount > (wallet.balance || 0)) { warningDiv.textContent = '❌ الرصيد غير كافي.'; warningDiv.classList.remove('hidden'); return; }
        if ((wallet.daily_used + amount) > wallet.daily_limit) { warningDiv.textContent = `❌ تتجاوز الحد اليومي (${formatCurrency(wallet.daily_limit)}).`; warningDiv.classList.remove('hidden'); return; }
        if ((wallet.monthly_used + amount) > wallet.monthly_limit) { warningDiv.textContent = `❌ تتجاوز الحد الشهري (${formatCurrency(wallet.monthly_limit)}).`; warningDiv.classList.remove('hidden'); return; }
    }
    ...
    try {
        if (type === 'withdraw') {
            await updateDoc(doc(db, "wallets", walletId), {
                ...
                daily_used: wallet.daily_used + amount,
                monthly_used: wallet.monthly_used + amount,
            });
        } else {
...
function loadReports() {
    ...
    const totalPartsCost = globalRepairs.reduce((s, r) => s + (r.spare_part_cost || 0), 0);
    const totalTechFees = globalRepairs.reduce((s, r) => s + (r.technician_fee || 0), 0);
    const totalExpensesVal = globalExpenses.reduce((s, e) => s + (e.amount || 0), 0);
    ...
    globalRepairs.forEach(r => {
        ...
        techMap[r.technician].revenue += (r.repair_price || 0);
    });
    document.getElementById('technician-performance').innerHTML = Object.values(techMap).map((t, i) => `
    ...
    globalRepairs.forEach(r => {
        ...
        if (!custMap[r.customer_name]) custMap[r.customer_name] = { name: r.customer_name, total: 0 };
        custMap[r.customer_name].total += (r.repair_price || 0);
    });
    document.getElementById('top-customers').innerHTML = Object.values(custMap).sort((a, b) => b.total - a.total).slice(0, 8).map((c, i) => `
        <div class="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2.5 mb-2">
            ...
            <div class="font-bold text-blue-600">${formatCurrency(c.total)}</div>
        </div>
    `).join('') || '<p class="text-gray-400 text-center py-4">لا توجد بيانات</p>';
}
...
function loadAlerts() {
    ...
    const lowStockAlerts = globalParts.filter(p => p.min_quantity && p.quantity <= p.min_quantity).map(p => ({
        type: 'stock', title: `مخزون منخفض: ${p.name}`, desc: `الكمية المتبقية ${p.quantity} (الحد الأدنى: ${p.min_quantity})`, severity: p.quantity === 0 ? 'critical' : 'warning', icon: 'fa-box', color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-r-amber-400',
    }));
    const overdueAlerts = globalRepairs.filter(r => r.delivery_date && r.status !== 'تم_التسليم' && new Date(r.delivery_date) < now).map(r => ({
        type: 'overdue', title: `تأخر تسليم: ${r.device_name}`, desc: `العميل: ${r.customer_name}`, severity: 'critical', icon: 'fa-clock', color: 'text-red-600', bg: 'bg-red-50', border: 'border-r-red-500',
    }));
    ...
    `;
    document.getElementById('alerts-list').innerHTML = allAlerts.length > 0 ? allAlerts.map(a => `
        <div class="card ${a.bg} border-r-4 ${a.border}"><div class="card-body"><div class="flex items-start gap-3">
    ...
    document.getElementById('alerts-count').textContent = allAlerts.length;
    document.getElementById('alerts-count').style.display = allAlerts.length > 0 ? 'inline-block' : 'none';
}
...
function loadSubscriptions() {
    ...
    const totalRevenue = globalSubscriptions.reduce((s, sub) => s + (sub.price || 0), 0);
    ...
    document.getElementById('subscription-summary-cards').innerHTML = `
        <div class="stat-card"><div class="stat-card-icon icon-green"><i class="fas fa-check-circle"></i></div><p class="stat-card-title">اشتراكات نشطة</p><p class="stat-card-value">${active}</p></div>
        <div class="stat-card"><div class="stat-card-icon icon-blue"><i class="fas fa-dollar-sign"></i></div><p class="stat-card-title">إيرادات الاشتراكات</p><p class="stat-card-value">${formatCurrency(totalRevenue)}</p></div>
    ...
    let filtered = globalSubscriptions.filter(s => {
        const match = !search || s.customer_name?.toLowerCase().includes(search) || s.customer_email?.toLowerCase().includes(search);
    ...
    document.getElementById('subscriptions-table-body').innerHTML = filtered.map((s, i) => {
        ...
        return `<tr>
            ...
            <td class="font-semibold">${s.customer_name}</td><td class="text-sm">${s.customer_email}</td>
...
function openSubscriptionModal(subId = null) {
    ...
    select.innerHTML = '<option value="">اختر مستخدم</option>' + globalUsers.map(u => `<option value="${u.id}">${u.name || u.email} (${u.email})</option>`).join('');
    ...
    if (subId) {
        ...
        if (s) {
            ...
            if (s.linked_user_id) document.getElementById('subscription-linked-user').value = s.linked_user_id;
        }
    } else {
...
function onLinkedUserChange() {
    ...
    if (userId) {
        ...
        if (user) {
            document.getElementById('subscription-customer-name').value = user.name || '';
...
function renderTechnicians() {
    document.getElementById('technicians-list').innerHTML = globalTechnicians.map((t, i) => `
...
function addTechnician() {
    ...
    if (input.value.trim()) { globalTechnicians.push(input.value.trim()); input.value = ''; renderTechnicians(); updateTechSelects(); saveTechniciansToDB(); }
}

function removeTechnician(index) { globalTechnicians.splice(index, 1); renderTechnicians(); updateTechSelects(); saveTechniciansToDB(); }

async function saveTechniciansToDB() { await updateDoc(doc(db, "settings", ownerId), { technicians: globalTechnicians }); }
...
function confirmDelete(type, id) {
    ...
    document.getElementById('delete-message').textContent = `هل أنت متأكد من حذف ${labels[type] || ''} "${name || ''}"؟ لا يمكن التراجع عن هذا.`;
