...
function bindEvents() {
    ...
    if (shopName) shopName.addEventListener('input', updateInvoicePreview);
    
    const ownerName = document.getElementById('set-owner-name');
    if (ownerName) ownerName.addEventListener('input', updateInvoicePreview);
    
    const phone = document.getElementById('set-phone');
    if (phone) phone.addEventListener('input', updateInvoicePreview);
    
    const address = document.getElementById('set-address');
    if (address) address.addEventListener('input', updateInvoicePreview);
}
...
function loadDashboard() {
    ...
    if (globalUsers.length > 0) {
        document.getElementById('users-manager').innerHTML = globalUsers.map(u => `
...
function loadExpensesTable() {
    ...
    `;
    document.getElementById('expenses-list').innerHTML = filtered.length ? filtered.map(e => `
...
function loadCustomersTable() {
    ...
    document.getElementById('customers-list').innerHTML = customers.length ? customers.map(c => `
...
function loadWallets() {
    ...
    document.getElementById('wallets-table-body').innerHTML = globalWallets.length ? globalWallets.map(w => {
    ...
    document.getElementById('wallet-transactions-body').innerHTML = sorted.length ? sorted.slice(0, 20).map(t => {
...
function loadSubscriptions() {
    ...
    document.getElementById('subscriptions-table-body').innerHTML = filtered.length ? filtered.map((s, i) => {
...
function loadSettings() {
    ...
    const setWarrantyDays = document.getElementById('set-warranty-days');
    const setWarrantyNotes = document.getElementById('set-warranty-notes');
    const setLanguage = document.getElementById('set-language');
    
    if (setShopName) setShopName.value = globalSettings.shop_name || '';
    if (setOwnerName) setOwnerName.value = globalSettings.owner_name || '';
    if (setPhone) setPhone.value = globalSettings.phone || '';
    if (setAddress) setAddress.value = globalSettings.address || '';
    if (setWarrantyDays) setWarrantyDays.value = globalSettings.warranty_days || 30;
    if (setWarrantyNotes) setWarrantyNotes.value = globalSettings.warranty_notes || '';
    if (setLanguage) setLanguage.value = globalSettings.language || 'ar';
...
function updateInvoicePreview() {
    ...
    if (previewShopName) previewShopName.textContent = setShopName ? setShopName.value || 'اسم المحل' : 'اسم المحل';
    if (previewOwner) previewOwner.textContent = setOwnerName ? setOwnerName.value || '' : '';
    if (previewPhone) previewPhone.textContent = setPhone && setPhone.value ? '📞 ' + setPhone.value : '';
    if (previewAddress) previewAddress.textContent = setAddress && setAddress.value ? '📍 ' + setAddress.value : '';
}
