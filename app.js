...
function loadUsersManager() {
    ...
    container.innerHTML = `
        <div class="flex justify-between items-center mb-4">
            <p class="text-sm font-bold text-gray-600">👥 إدارة المستخدمين والصلاحيات</p>
            <span class="badge badge-blue text-xs">${globalUsers.length} مستخدم</span>
        </div>
        <div class="space-y-2">
            ${globalUsers.length > 0 ? globalUsers.map(u => `
                <div class="flex justify-between items-center bg-white border border-gray-100 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow">
                    <div class="flex items-center gap-3">
                        <img src="${u.photoURL || u.avatar || ''}" class="w-10 h-10 rounded-full bg-blue-100" onerror="this.style.display='none'">
                        <div>
                            <span class="font-semibold text-sm block">${u.fullName || u.name || 'مستخدم'}</span>
                            <span class="text-xs text-gray-400">${u.email}</span>
                            <span class="text-xs text-gray-400 block">آخر دخول: ${u.lastLogin ? new Date(u.lastLogin.seconds * 1000).toLocaleDateString('ar-EG') : '—'}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="text-center">
                            <span class="badge ${u.role === 'admin' ? 'badge-purple' : u.isApproved ? 'badge-green' : 'badge-red'} text-xs">
                        ...
                        ${u.role !== 'admin' ? `
                            <div class="flex gap-1">
                                ...
                                <button class="btn-icon text-red" onclick="confirmDelete('user','${u.id}')" title="حذف">
...
function loadDashboard() {
    ...
    // إخفاء الرسوم البيانية (بناءً على طلبك)
...
// تغيير حالة الجهاز مباشرة من الجدول
...
// ================================
// 6. المحافظ الإلكترونية (مع إمكانية التعديل)
// ================================
