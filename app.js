...
async function initApp() {
    ...
    // إخفاء التحميل سريعاً
    setTimeout(() => { if (overlay) overlay.classList.remove('show'); }, 500);
}
...
// ================================
// إدارة صلاحيات المستخدمين (للأدمن فقط) - تصميم احترافي
// ================================
function loadUsersManager() {
    ...
    container.innerHTML = `
        <div class="flex justify-between items-center mb-3">
            <div>
                <p class="font-bold text-sm text-gray-800">👥 المستخدمين والصلاحيات</p>
                <p class="text-xs text-gray-400">${globalUsers.length} مستخدم مسجل</p>
            </div>
            <span class="badge badge-blue">${globalUsers.filter(u => u.isApproved).length} مفعل</span>
        </div>
        <div class="space-y-2">
            ${globalUsers.length > 0 ? globalUsers.map(u => {
                const initials = (u.fullName || u.name || u.email || '?').charAt(0).toUpperCase();
                return `
                <div class="user-card">
                    <div class="user-card-avatar">${initials}</div>
                    <div class="user-card-info">
                        <div class="user-card-name">${u.fullName || u.name || 'مستخدم'}</div>
                        <div class="user-card-email">${u.email}</div>
                    </div>
                    <div class="user-card-meta">
                        <span class="badge ${u.role === 'admin' ? 'badge-purple' : u.isApproved ? 'badge-green' : 'badge-red'} mb-1">
                            ${u.role === 'admin' ? '👑 مدير' : u.isApproved ? '✅ مفعل' : '⏳ معلق'}
                        </span>
                        <div class="user-card-date">${u.lastLogin ? new Date(u.lastLogin.seconds * 1000).toLocaleDateString('ar-EG') : '—'}</div>
                    </div>
                    ${u.role !== 'admin' ? `
                    <div class="flex gap-1">
                        <button class="btn-xs ${u.isApproved ? 'btn-danger' : 'btn-success'}" 
                            onclick="toggleUserApproval('${u.id}', ${u.isApproved})">
                            ${u.isApproved ? '🚫' : '✅'}
                        </button>
                        <button class="btn-xs btn-danger" onclick="confirmDelete('user','${u.id}')">🗑️</button>
                    </div>` : ''}
                </div>`;
            }).join('') : '<p class="text-center text-gray-400 py-4">لا يوجد مستخدمين</p>'}
        </div>
    `;
}
...
// ================================
// 2. أوامر الصيانة (مع تغيير الحالة السريع والطباعة)
// ================================
...
async function printRepairInvoice(repairId) {
    ...
    printWindow.document.write(`
        <html dir="rtl">
        <head>
            <title>فاتورة صيانة - ${repair.device_name}</title>
            <style>
                ...
                .invoice-header h1 { color: #2563eb; font-size: 24px; margin-bottom: 4px; }
                .invoice-header p { color: #64748b; font-size: 14px; margin: 2px 0; }
                .invoice-id { background: #dbeafe; color: #1e40af; padding: 6px 12px; border-radius: 8px; display: inline-block; font-weight: 700; margin: 8px 0; }
                .info-row { display: flex; justify-content: space-between; margin-bottom: 12px; padding: 8px 0; border-bottom: 1px solid #e2e8f0; }
                .info-row span:first-child { color: #64748b; font-size: 13px; }
                .info-row span:last-child { font-weight: 700; font-size: 14px; }
                .amount { font-size: 20px; color: #2563eb; font-weight: 800; text-align: center; margin: 16px 0; }
                .warranty { background: #fef3c7; border: 1px solid #fde68a; border-radius: 10px; padding: 12px; margin-top: 20px; font-size: 13px; }
                .technician-stamp { text-align: left; margin-top: 30px; color: #64748b; }
        ...
        <body>
            <div class="invoice-header">
                ...
                ${phone ? `<p>📞 ${phone}</p>` : ''}
                ${address ? `<p>📍 ${address}</p>` : ''}
                <div class="invoice-id">فاتورة صيانة #${repairId.slice(0, 6).toUpperCase()}</div>
                <p style="font-size:12px;color:#94a3b8;">تاريخ: ${new Date().toLocaleDateString('ar-EG')}</p>
            </div>
            
            <div class="info-row"><span>العميل</span><span>${repair.customer_name || '—'}</span></div>
            <div class="info-row"><span>رقم الهاتف</span><span>${repair.customer_phone || '—'}</span></div>
            <div class="info-row"><span>الجهاز</span><span>${repair.device_name || '—'}</span></div>
            <div class="info-row"><span>الفني</span><span>${repair.technician || '—'}</span></div>
            <div class="info-row"><span>تاريخ الاستلام</span><span>${repair.receive_date || '—'}</span></div>
            <div class="info-row"><span>تاريخ التسليم</span><span>${repair.delivery_date || 'قيد الصيانة'}</span></div>
            <div class="info-row"><span>المشكلة</span><span>${repair.device_issue || '—'}</span></div>
            ${repair.spare_part_name ? `<div class="info-row"><span>قطع الغيار</span><span>${repair.spare_part_name} (${formatCurrency(repair.spare_part_cost)})</span></div>` : ''}
            
            <div class="amount">💰 إجمالي المبلغ: ${formatCurrency(repair.repair_price)}</div>
            
            <div class="warranty">
                <strong>🛡️ معلومات الضمان</strong><br>
                مدة الضمان: ${warrantyDays} يوم<br>
                ${warrantyNotes ? `ملاحظات: ${warrantyNotes}` : ''}
            </div>
            
            <div class="technician-stamp">
                <p>توقيع الفني: _______________</p>
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex gap-1">
                            ...
                            <button class="btn-icon text-green" onclick="printRepairInvoice('${r.id}')" title="طباعة"><i class="fas fa-print"></i></button>
    ...
    if (transactionsBody) {
        transactionsBody.innerHTML = sorted.length ? sorted.slice(0, 20).map(t => {
            ...
            return `<tr>
                ...
                <td class="font-bold transaction-amount ${t.type === 'deposit' ? 'deposit' : 'withdraw'}">${t.type === 'deposit' ? '+' : '-'} ${formatCurrency(t.amount)}</td>
...
function loadReports() {
    ...
    if (kpiEl) {
        kpiEl.innerHTML = `
            ...
            <div class="stat-card"><p class="stat-card-title">المصاريف التشغيلية</p><p class="stat-card-value">${formatCurrency(totalExpensesVal)}</p><p class="stat-card-sub">${globalExpenses.length} مصروف</p></div>
            ...
            <div class="stat-card"><p class="stat-card-title">متوسط قيمة الطلب</p><p class="stat-card-value">${formatCurrency(avgOrderValue)}</p><p class="stat-card-sub">لكل أمر صيانة</p></div>
            <div class="stat-card"><p class="stat-card-title">معدل إتمام الصيانة</p><p class="stat-card-value">${completionRate}%</p><p class="stat-card-sub">${completedOrders} من ${globalRepairs.length}</p></div>
        `;
    }
    ...
    if (techPerfEl) {
        techPerfEl.innerHTML = Object.values(techMap).length ? Object.values(techMap).map((t, i) => `
            <div class="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2.5 mb-2">
                ...
                <div class="flex-1"><p class="font-semibold">${t.name}</p><p class="text-xs text-gray-500">${t.completed} مكتمل من ${t.orders}</p></div>
...
async function saveSubscription(e) {
    ...
    try {
        ...
        // ⭐ تفعيل المستخدم تلقائياً عند إنشاء اشتراك
