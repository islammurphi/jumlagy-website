    ...
    if (session && session.role !== 'admin' && session.end_date) {
        ...
        if (dl <= 30) {
            subBadge = `<div class="subscription-banner ${dl <= 0 ? 'danger' : 'warning'}" style="display:block;margin-bottom:16px;">
                ⏳ ${dl > 0 ? `متبقي ${dl} يوم على انتهاء الاشتراك` : 'انتهى الاشتراك'}
            </div>`;
        }
    }
...
async function printRepairInvoice(repairId) {
    ...
    printWindow.document.write(`
        ...
        <style>@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;800&display=swap');body{font-family:'Tajawal',sans-serif;padding:30px}h1{color:#2563eb}table{width:100%;border-collapse:collapse}td{padding:8px;border-bottom:1px solid #eee}.total{font-size:20px;color:#2563eb;font-weight:800}@media print{body{padding:10px}}</style></head>
    ...
    if (transactionsBody) {
        transactionsBody.innerHTML = sorted.length ? sorted.slice(0, 20).map(t => {
            ...
            return `<tr>
                ...
                <td>
                    <button class="btn-icon" onclick="editTransaction('${t.id}')"><i class="fas fa-pen"></i></button>
                    <button class="btn-icon text-red" onclick="deleteTransaction('${t.id}')"><i class="fas fa-trash"></i></button>
                </td></tr>`;
        }).join('') : '<tr><td colspan="6" class="text-center py-6 text-gray-400">لا توجد عمليات</td></tr>';
    }
}
            ...
            return `<tr>
                ...
                <td>
                    <div class="flex gap-1">
                        <button class="btn-icon" onclick="openSubscriptionModal('${s.id}')" title="تعديل الاشتراك"><i class="fas fa-pen"></i></button>
...
function openSubscriptionModal(subId = null) {
    ...
    if (endDateField) endDateField.removeAttribute('readonly');
}
